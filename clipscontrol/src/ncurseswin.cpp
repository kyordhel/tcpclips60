...
void NCursesWin::execute(const std::string& cmd, const std::string& args){
	if(commandHandler) commandHandler(cmd, args);
}
...
void NCursesWin::addCommandHandler(const commandfunc& f){
	commandHandler = f;
}
